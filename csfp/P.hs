...
split3 :: [a] -> [[[a]]]
split3 (x:y:z:[]) =  [[x]:[y]:[z]:[]]
split3 (x:xs) =  ( map (\(ys,zs) -> [[x],ys,zs] ) ( split2' xs ) )
		++ [ [x:ys,vs,ws]   | (ys,zs) <- split2' xs, (vs,ws) <- split2' zs ]
...
firstS :: Eq a => a -> [a] -> Bool
firstS = \s (x:xs) -> s == x
-- symbol' :: Eq a => a -> Parser a a
-- symbol' = satisfy ( \c (x:_) -> x == c )
-- symbol' s xs = satisfy ( firstS s xs )
