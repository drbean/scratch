...
npINT :: NP -> World -> Entity -> Bool
-- npINT SnowWhite = iSnowWhite
npINT Everyone  = iPerson
-- npINT Someone  = filter (\y -> iPerson i y) entities
-- npINT (NP1 det cn)  = iDET det (iCN cn) 
-- npINT (NP2 det rcn) = iDET det (iRCN rcn) 

intensNP :: NP -> (IEntity -> IBool) -> IBool
intensNP = iPropToB . npINT
-- intensNP = iPropToB . iNP
...
-- intensSent :: Sent -> IBool
-- intensSent (Sent (DET det ) (CN cn) vp) = ( iDET det) (intensCN cn) (intensVP vp)
