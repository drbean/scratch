...
treeOfNumbers = concat ( map (\n -> [ (n-x, x) | x <- [0..n] ] ) [0..] )

type Quant = (Int -> Bool) -> [Int] -> Bool
-- genTree :: Quant -> [(Int, Int)]
-- genTree p = filter (uncurry p) treeOfNumbers
-- genTree all = filter (\(m,_) -> m == 0 ) treeOfNumbers
-- genTree any = filter (\(_,n) -> n /= 0 ) treeOfNumbers
-- genTree f = \f -> filter f treeOfNumbers
...
-- intDET AtLeast n p q = length filter q ( filter p entities ) > n
-- AtLeast :: Int -> ( Entity -> Bool ) -> ( Entity -> Bool ) -> Bool
atLeast n p q = length ( filter q ( filter p entities ) ) >= n
-- intDET AtMost n p q = atLeast n-1 p ( not q )
atMost :: Int -> ( Entity -> Bool ) -> ( Entity -> Bool ) -> Bool
atMost n p q = length ( filter q ( filter p entities ) ) <= n
